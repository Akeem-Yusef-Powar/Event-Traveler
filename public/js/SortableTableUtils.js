...
SortableTableUtils.makeText = function (text) {
    ...
    tableData.style.textAlign = "center"; // text elements usually align left
    return tableData;
};
...
SortableTableUtils.makeLink = function (link) {

    var tableData = document.createElement('td');
    link = link || ""; // set it to empty string if the input parameter does not exist
    if (link === "") {
        tableData.sortOrder = -1;  // put empty entries at top if sorting by this column
    } else {
        tableData.sortOrder = link.toUpperCase();
    }
    link = "<a href='" + link + "'>link</a>";
    //link = "<form><button formaction=\"${link}\">Link</button></form>"
    //link = "<button onclick=\"location.href='${link}'\" type=\"button\">Link</button>"
    tableData.innerHTML = link;
    tableData.style.textAlign = "center"; // text elements usually align left
    return tableData;
};
